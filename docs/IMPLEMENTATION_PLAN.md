...
쓰기는 **최고 기록이 갱신될 때만** 이 문서를 건드리므로, 한 반에서 초당 1회 수준을
넘지 않는다(Firestore 문서 쓰기 한도 안).

모든 문서에 `game: "dodge"` 필드를 처음부터 넣는다 (PRD 12의 4주차 러너 게임 연계).
쿼리는 전부 단일 필드 조건만 쓰도록 짜서 **복합 색인을 만들 필요가 없다**.
...
│  ├─ session.js               # 교사 세션 쿠키
│  ├─ guard.js                 # 교사 API 가드
│  ├─ secret.js                # 서명 키 (환경변수 없으면 배포 해시에서 파생)
...
- [ ] 실제 Firestore 프로젝트에 붙여 스모크 테스트 32개 항목 통과 (`npm run test:smoke`)

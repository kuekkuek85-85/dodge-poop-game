...
/** 되돌아온 라운드 토큰을 알아보기 위해 학생별로 기억해 두는 개수 */
const RECENT_RUN_IDS = 20;
...
const memoryStore = {
  async submitAttempt({ identity, attempt, runId, flagged, flagReason, now }) {
    ...
    const consumed = prev ? prev.recentRunIds || [] : [];
    if (runId && consumed.includes(runId)) return duplicateResult(prev, attempt);
    ...
    if (runId) student.recentRunIds = [...consumed, runId].slice(-RECENT_RUN_IDS);
  ...
  async submitAttempt({ identity, attempt, runId, flagged, flagReason, now }) {
    ...
    return db.runTransaction(async (tx) => {
      ...
      const student = {
        ...
        lastSubmitAt: now,
        recentRunIds: runId ? [...consumed, runId].slice(-RECENT_RUN_IDS) : consumed,
      };

 ...
 *
 * 초기화는 순위표에 시각을 남긴 뒤 학생 문서를 지우는데, 그 사이에 초기화 후
 * 시작한 판이 커밋되면 옛 최고 기록을 그대로 들고 새 회차로 넘어간다. 그러면
 * 순위표는 비어 있는데 학생 문서만 옛 기록을 간직한 상태가 된다.
 */
function forgetIfReset(prev, boardResetAt) {
  if (!prev) return null;
  if (!boardResetAt || boardResetAt <= (prev.lastSubmitAt || 0)) return prev;
  return null; // 초기화 이후 첫 저장 — 처음 보는 학생처럼 다룬다
}
...
const memoryStore = {
  async submitAttempt({ identity, attempt, runId, runIssuedAt, flagged, flagReason, now }) {
    ...
    const boardResetAt = mem.resets.get(identity.classNo) || 0;
    const clearedAt = Math.max(stored ? stored.clearedAt || 0 : 0, boardResetAt);
    if (startedBeforeClear(runIssuedAt, clearedAt)) return clearedResult();

    // 초기화 이후 첫 저장이면 남아 있는 이전 집계는 버린다
    const prev = forgetIfReset(stored, boardResetAt);
  ...
  async recomputeStudent(studentKey) {
    ...
    const clearedAt = Math.max(student.clearedAt || 0, mem.resets.get(student.classNo) || 0);
...
const firestoreStore = {
  async submitAttempt({ identity, attempt, runId, runIssuedAt, flagged, flagReason, now }) {
    ...
    return db.runTransaction(async (tx) => {
      ...
      const boardResetAt = boardData ? boardData.resetAt || 0 : 0;
      const clearedAt = Math.max(stored ? stored.clearedAt || 0 : 0, boardResetAt);
      if (startedBeforeClear(runIssuedAt, clearedAt)) return clearedResult();

      // 초기화 이후 첫 저장이면, 아직 지워지지 않은 이전 집계는 버린다
      const prev = forgetIfReset(stored, boardResetAt);
  ...
  async recomputeStudent(studentKey) {
    ...
    await db.runTransaction(async (tx) => {
      ...
      const clearedAt = Math.max(student.clearedAt || 0, boardSnap.exists ? boardSnap.data().resetAt || 0 : 0);
